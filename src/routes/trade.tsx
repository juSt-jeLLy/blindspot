...
function Trade() {
  ...
  async function refreshFundingInfo() {
    ...
    try {
      ...
      // Auto-repair on refresh: run missing steps in order if we can.
      if (isZeroHandle(info.confidentialBalanceHandle)) {
        const approved = Number(info.underlyingAllowanceToWrapper || "0");
        const requested = Number(fundAmount || "0");
        const repairAmount = requested > 0 ? requested : approved;

        if (repairAmount > 0 && approved < repairAmount) {
          setStepStatus((s) => ({ ...s, approve: "Approving underlying token..." }));
          await approveUnderlyingForWrapper({
            underlyingToken: funding.underlyingToken,
            wrapperToken: funding.cToken,
            amountHuman: String(repairAmount),
          });
          setStepStatus((s) => ({ ...s, approve: "✓ Underlying approved" }));
        }

        if (repairAmount > 0) {
          setStepStatus((s) => ({ ...s, wrap: `Wrapping ${repairAmount} to confidential token...` }));
          await wrapIntoConfidential({ wrapperToken: funding.cToken, amountHuman: String(repairAmount) });
          setStepStatus((s) => ({ ...s, wrap: "✓ Wrapped to confidential token" }));
        } else {
          setStepStatus((s) => ({
            ...s,
            wrap: "No amount to wrap. Enter Funding Amount or approve underlying first.",
          }));
        }

        info = await getFundingStatus({
          underlyingToken: funding.underlyingToken,
          cToken: funding.cToken,
          escrow: pair.escrow,
          owner,
      ...
      if (!info.escrowOperatorEnabled) {
        setStepStatus((s) => ({ ...s, escrow: "Enabling escrow permission..." }));
        await approveConfidentialForEscrow({ cToken: funding.cToken, escrow: pair.escrow, amountHuman: fundAmount || "1" });
      ...
      if (String(msg).includes("0xbf376c7a")) {
        setFundingError("wrap failed on selected route. This usually means token route mismatch (e.g. WETH wrapper selected while funding token is USDC). Switch pair/side and refresh again.");
      } else {
        setFundingError(msg);
      }
      setFundingInfo(null);
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8">
      ...
      <form onSubmit={submit} className="space-y-4 rounded border border-border bg-card p-6">
        <div className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground">Funding token for current side: {fundingTokenSymbol ?? fundingToken}</div>
        ...
        <div className="rounded border border-dashed border-border bg-background/30 p-4 text-sm">
          ...
          <div className="flex items-center justify-between"><span className="text-muted-foreground">Funding Token</span><span>{fundingTokenSymbol ?? fundingToken}</span></div>
          ...
          <div className="flex items-center justify-between"><span className="text-muted-foreground">Underlying Balance</span><span>{fundingInfo?.underlyingBalance ?? "—"} {fundingTokenSymbol ?? fundingToken}</span></div>
          <div className="flex items-center justify-between"><span className="text-muted-foreground">Approved to Wrapper</span><span>{fundingInfo?.underlyingAllowanceToWrapper ?? "—"} {fundingTokenSymbol ?? fundingToken}</span></div>
          <div className="flex items-center justify-between"><span className="text-muted-foreground">Available for Trading (confidential)</span><span>{fundingInfo?.confidentialBalance ?? "—"} c{fundingTokenSymbol ?? fundingToken}</span></div>
